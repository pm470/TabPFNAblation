...
def main(argv=None):
    ...
    for split in ["all", "id", "ood"]:
        ...
        baseline_roc = roc_df["baseline_mean"].iloc[0] if not roc_df.empty else float("nan")  # pyright: ignore
        baseline_log = log_df["baseline_mean"].iloc[0] if not log_df.empty else float("nan")  # pyright: ignore
